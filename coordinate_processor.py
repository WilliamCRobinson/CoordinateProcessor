...
def combine_to_master(coordmastername, csvdir, masterdirectory):
    """
    ...
    :param masterdirectory: the directory where the master files are being saved, CSV and PNG files included.
    :return: null, generates files
    """
    os.chdir(csvdir)
    ext = "csv"
...
def master_heatmap(master_csv_file, xlabels, ylabels, axis):
    ...
    :param axis: specifies the coordinate axis corresponding to this heatmap
...
def main():
    ...
    os.mkdir(mastdir)
    os.chdir(mastdir)
    main_csv_name = os.getcwd().replace("\\", "_").replace(":", "_").replace("/", "_")
    os.chdir(cwd)
    combine_to_master("x_" + main_csv_name, xvcdir, mastdir)
    os.chdir(cwd)
    combine_to_master("y_" + main_csv_name, yvcdir, mastdir)
    os.chdir(cwd)
    combine_to_master("z_" + main_csv_name, zvcdir, mastdir)
    os.chdir(cwd)
    # Make heatmaps
    os.chdir(mastdir)
    master_heatmap("x_" + main_csv_name + ".csv", time_bin_edges, data_bin_edges, "x")
    master_heatmap("y_" + main_csv_name + ".csv", time_bin_edges, data_bin_edges, "y")
    master_heatmap("z_" + main_csv_name + ".csv", time_bin_edges, data_bin_edges, "z")
